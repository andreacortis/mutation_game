...
positive and 3 negative. But these are vectors in :math:`\mathbb{R}^2` (our
simple root basis). To see the geometry, we need to place them in a real
coordinate system.

**Step 1: The standard basis vectors.** In :math:`\mathbb{R}^3`, define three
...
**Step 2: Form the roots.** The roots of :math:`A_2` are all differences
:math:`e_i - e_j` where :math:`i \neq j`. There are :math:`3 \times 2 = 6`
such differences:

.. math::

   \begin{array}{ll}
   \text{Positive:} & e_1 - e_2 = (1, -1, 0) \\
                     & e_1 - e_3 = (1, 0, -1) \\
                     & e_2 - e_3 = (0, 1, -1) \\[6pt]
   \text{Negative:} & e_2 - e_1 = (-1, 1, 0) \\
                     & e_3 - e_1 = (-1, 0, 1) \\
                     & e_3 - e_2 = (0, -1, 1)
   \end{array}

Notice that each negative root is just the opposite direction of a positive
root: :math:`-(e_1 - e_2) = e_2 - e_1`.

**Step 3: Project to 2D.** All 6 vectors satisfy :math:`x + y + z = 0`
(each component sums to zero). That means they all lie on a flat plane
through the origin inside :math:`\mathbb{R}^3`. This plane is 2-dimensional,
so we can draw the roots in 2D by choosing two perpendicular directions
within the plane.

**Step 4: See the hexagon.** When we plot these 6 points in the plane, they
form a perfect **regular hexagon** — all points at the same distance from the
origin, equally spaced at 60-degree intervals:
...
**Step 5: The symmetry group.** The hexagon has the symmetry of an
...
which has :math:`3! = 6` elements (3 rotations + 3 reflections). This is
exactly the **Weyl group** of :math:`A_2`, the same group generated by the
two mutation matrices.

We can verify all of this with the library:
...
.. code-block:: pycon
   ...
   >>> # The Dynkin diagram: 2 nodes, one edge
   >>> print(game.adj)
   [[0 1]
    [1 0]]
